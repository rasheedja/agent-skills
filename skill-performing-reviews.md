...
This skill describes how to evaluate PR review comments critically, decide whether to address them or respond with a rationale, and how to format replies (including identifying the reply as from an agent). For the mechanics of reading comments and posting replies via `gh` CLI/API, use **[skill-gh-pr-review-comments.md](skill-gh-pr-review-comments.md)**. For **addressing unresolved comments** in a single pass (check PR → address and resolve each unresolved thread), use **[skill-pr-review-address-unresolved.md](skill-pr-review-address-unresolved.md)**. For the **GitHub Copilot loop** (request Copilot review → wait for comments → address and resolve → re-request Copilot → repeat until no new comments), use **[skill-pr-review-loop-copilot.md](skill-pr-review-loop-copilot.md)**. For the **subagent review → main agent address** flow (one subagent reviews, main agent triages and makes changes, loop until no further comments; review files not committed), use **[skill-subagent-review-main-agent-address.md](skill-subagent-review-main-agent-address.md)**.
