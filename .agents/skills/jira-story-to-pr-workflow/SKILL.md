---
name: jira-story-to-pr-workflow
description: "Implement a Jira story directly using a test-driven workflow through a draft PR. Use jira-to-pr-review when a separate implementor and orchestrator review loop are requested."
---
...
For an orchestrator that reviews a separately selected implementor's work, use [jira-to-pr-review](../jira-to-pr-review/SKILL.md), also called **jira-to-pr-review**. That workflow keeps implementation and all corrections with the selected implementor.

**Prerequisites:** **[jira-acli](../jira-acli/SKILL.md)** (Jira via `acli`), **[commits-and-pre-commit-checks](../commits-and-pre-commit-checks/SKILL.md)** (conventional commits, pre-commit checks), **[jujutsu](../jujutsu/SKILL.md)** (if using jj), and **[pr-title-and-description](../pr-title-and-description/SKILL.md)** (PR title/body). For opening a PR you need `gh` (GitHub CLI) or equivalent.
...
- **Dependencies:** Does the story reference blockers, other tickets, or external systems? If blocked or blocked-by is set, check **[jira-acli](../jira-acli/SKILL.md)** for `acli jira workitem link list --key <KEY>` and assess whether you can still proceed.
...
- **Branch (git)** or **bookmark (jj)** name — **the repo's own convention takes precedence** (its `CLAUDE.md` / `AGENTS.md`, or a user instruction; e.g. realfi uses `feature/<jira-ticket>-<name>`). See **[jujutsu](../jujutsu/SKILL.md)**. Only when the repo states none, use the personal default:
...
See **[jujutsu](../jujutsu/SKILL.md)** for the full sequence (e.g. creating the change, moving the bookmark, leaving working copy on a new empty change after commits).
...
- **Run tests and project checks** before every commit. Follow **[commits-and-pre-commit-checks](../commits-and-pre-commit-checks/SKILL.md)**: discover and run the project’s test/lint/build commands (Makefile, `npm run test`, `cargo test`, etc.) and fix failures before committing.

---

## 7. Commits (conventional) and VCS

- **Conventional commits:** Every commit must follow `<type>(<scope>): <short description>`. Use `feat` for new behaviour, `fix` for bug fixes, `test` for test-only changes. Add a body with bullet points when it helps. See **[commits-and-pre-commit-checks](../commits-and-pre-commit-checks/SKILL.md)**.
- **Git:** `git add` and `git commit -m "type(scope): description"` (and optional `-m "body"`). Prefer small, logical commits.
- **Jujutsu:** Use `jj new`, edit, then `jj describe -m "..."` and `jj bookmark set rasheedja/<STORY_KEY>/<short-description> -r @` so the bookmark stays at the tip; then **`jj new @`** so `@` is an empty change on top (avoids accidental amends). To adjust an older revision, prefer **`jj new <rev>`** + **`jj squash`** over **`jj edit`**. Use `jj split` when one change becomes multiple commits. See **[jujutsu](../jujutsu/SKILL.md)** §2, §9b.
...
- **Draft PR:** Prefer opening as **draft** so the user can review before marking ready.
  ...
  **GitHub CLI:**
  ...
  Title should follow the same style as commits (e.g. `feat(scope): short description (STORY_KEY)`). For the body, use **[pr-title-and-description](../pr-title-and-description/SKILL.md)**: bullet points summarizing what changed and why, and reference the Jira key.
...
9. **Commit:** Conventional commits; small logical commits; follow **[commits-and-pre-commit-checks](../commits-and-pre-commit-checks/SKILL.md)** and **[jujutsu](../jujutsu/SKILL.md)** if using jj.
...
| Topic | Skill / doc |
|-------|-------------|
| Jira (view, transition, links) | **[jira-acli](../jira-acli/SKILL.md)** |
| Conventional commits, pre-commit checks | **[commits-and-pre-commit-checks](../commits-and-pre-commit-checks/SKILL.md)** |
| Jujutsu (change, bookmark, push) | **[jujutsu](../jujutsu/SKILL.md)** |
| PR title and description | **[pr-title-and-description](../pr-title-and-description/SKILL.md)** |
