---
name: subagent-review-main-agent-address
description: "Run a PR review loop where a subagent reviews and the main agent triages and implements fixes. Use jira-to-pr-review for the opposite implementation ownership."
---
...
For **human PR review comments** (e.g. GitHub), see **[performing-reviews](../performing-reviews/SKILL.md)** and **[gh-pr-review-comments](../gh-pr-review-comments/SKILL.md)**. For **commit format and jj mechanics** when making changes, see **[commits-and-pre-commit-checks](../commits-and-pre-commit-checks/SKILL.md)** and **[jujutsu](../jujutsu/SKILL.md)**.
...
1. **Spawn a subagent** to perform the review.
   ...
   - Optionally instruct the subagent to use **[commits-and-pre-commit-checks](../commits-and-pre-commit-checks/SKILL.md)** and **[jujutsu](../jujutsu/SKILL.md)** if it suggests edits (small commits, conventional format); often the main agent will make the edits instead, so this is optional.

2. **Main agent reads the review output** and triages each recommendation (see §4).

3. **Main agent makes only the changes** that are justified (correct, in scope for this branch). Use **small, logical commits** and **conventional commit format**; commit on the branch (git, default: `git add` + `git commit`; jj: see **[jujutsu](../jujutsu/SKILL.md)** for `jj new`, **`jj new @`**, **`jj new` + `jj squash`** vs **`jj edit`**, `jj split`) and follow **[commits-and-pre-commit-checks](../commits-and-pre-commit-checks/SKILL.md)** (run project checks before committing).
...
- **Commit style (if the subagent might edit):** Reference **[commits-and-pre-commit-checks](../commits-and-pre-commit-checks/SKILL.md)** and **[jujutsu](../jujutsu/SKILL.md)** so any edits use conventional commits and small logical commits. If the main agent will do all edits, you can omit this.
...
**After all haiku edits:** the subagent's file edits land in the worktree as unstaged changes (git) or the current jj working copy (`@`). Stage and commit them onto the branch before pushing — git (default): `git add` + `git commit`; jj: `jj squash` to fold into the bookmark commit (**[jujutsu](../jujutsu/SKILL.md) §13**).
...
**After parallel subagents complete:** run `git diff --stat` (or `jj diff --stat`) to verify the total set of changed files looks correct, then commit them onto the branch — git (default): `git add` + `git commit`; jj: `jj squash` into the bookmark commit (**[jujutsu](../jujutsu/SKILL.md) §13**).
