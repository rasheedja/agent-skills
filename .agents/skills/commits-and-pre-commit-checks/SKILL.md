---
name: commits-and-pre-commit-checks
description: "Create small conventional commits and discover and run repository checks before committing with Git or Jujutsu."
---
...
5. In a **jj** repo: use **[jujutsu](../jujutsu/SKILL.md)** for `jj new`, `jj bookmark set`, **`jj new @`** after finishing a commit (empty working copy), `jj squash` when folding a fix into a parent revision, and `jj git push`. In a **git** repo: use `git add` and `git commit` (and optionally `git push` when appropriate).
...
| jj mechanics | See **[jujutsu](../jujutsu/SKILL.md)** for creating changes, bookmarks, push, empty working copy (`jj new @`), and **`jj new` + `jj squash`** vs **`jj edit`** (§2). |
