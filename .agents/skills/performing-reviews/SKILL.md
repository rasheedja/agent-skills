---
name: performing-reviews
description: "Triage incoming PR review comments, make justified fixes, and reply or resolve with accurate agent attribution when authorized."
---

# Skill: Performing code reviews and replying to review comments

This skill describes how to evaluate PR review comments critically, decide whether to address them or respond with a rationale, and how to format replies (including identifying the reply as from an agent). For the mechanics of reading comments and posting replies via `gh` CLI/API, use **[gh-pr-review-comments](../gh-pr-review-comments/SKILL.md)**. For **addressing unresolved comments** in a single pass (check PR → address and resolve each unresolved thread), use **[pr-review-address-unresolved](../pr-review-address-unresolved/SKILL.md)**. For the **GitHub Copilot loop** (request Copilot review → wait for comments → address and resolve → re-request Copilot → repeat until no new comments), use **[pr-review-loop-copilot](../pr-review-loop-copilot/SKILL.md)**. For the **subagent review → main agent address** flow (one subagent reviews, main agent triages and makes changes, loop until no further comments; review files not committed), use **[subagent-review-main-agent-address](../subagent-review-main-agent-address/SKILL.md)**.
...
- **In-thread replies** — Use the PR review comment reply flow so your message appears under the original comment. This keeps context and resolution in one place. Use the gh/API steps in **[gh-pr-review-comments](../gh-pr-review-comments/SKILL.md)** (list comments, get parent `id`, POST with `in_reply_to`).
...
1. **Fetch review comments** — See [gh-pr-review-comments](../gh-pr-review-comments/SKILL.md) §2. List comments for the PR and note each comment’s `id` and `body`. **Restrict to unresolved threads** (filter by resolution state if the API or tool supports it).
...
4. **Post in-thread replies** — For each comment you’re replying to (whether you changed code or not), use [gh-pr-review-comments](../gh-pr-review-comments/SKILL.md) §4: POST to `repos/…/pulls/<PR>/comments` with `body` and `in_reply_to`. In every reply body, start with **`Comment left by Cursor`** (or **`Comment left by agent`** / **`Reply by <AgentName>`** if the system provides a name).
5. **Resolve the threads** — After replying, resolve each thread you addressed so it no longer shows as “unresolved”. Use [gh-pr-review-comments](../gh-pr-review-comments/SKILL.md) §5 (GraphQL: get thread IDs from `reviewThreads`, then `resolveReviewThread`).
...
- **Resolve threads** once you’ve replied (and optionally made a code change); see [gh-pr-review-comments](../gh-pr-review-comments/SKILL.md) §5.
