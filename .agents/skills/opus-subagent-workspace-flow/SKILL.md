---
name: opus-subagent-workspace-flow
description: "Run the specialized Claude Opus implementation workflow in an isolated workspace through verification and draft PR delivery. Use delegated-implementation-review for configurable agent roles."
---
...
**Prerequisites:** **[jujutsu](../jujutsu/SKILL.md)** (jj change/bookmark/push mechanics — only when using jj), **[jira-story-to-pr-workflow](../jira-story-to-pr-workflow/SKILL.md)** (ticket reading, evaluation, transition, naming conventions), **[commits-and-pre-commit-checks](../commits-and-pre-commit-checks/SKILL.md)** (conventional commits, running checks), **[subagent-review-main-agent-address](../subagent-review-main-agent-address/SKILL.md)** (review loop after the subagent finishes).
...
- **Background** (`run_in_background: true`) is useful only if you have genuinely independent work to do in parallel (e.g. watching a long deploy). Be aware: background subagents cannot prompt for Bash permission approval and will stall silently (see **[subagent-review-main-agent-address](../subagent-review-main-agent-address/SKILL.md) §7**).
...
Apply the triage rules from **[subagent-review-main-agent-address](../subagent-review-main-agent-address/SKILL.md) §4**: accurate + in scope → keep; wrong → fix; out of scope → revert; already addressed → no change.
...
**Run project checks** before committing — build, tests, lint (see **[commits-and-pre-commit-checks](../commits-and-pre-commit-checks/SKILL.md)**). Do this from the workspace directory, not the default one.
...
(With jj: the subagent's file edits land in `@` as untracked changes — see **[jujutsu](../jujutsu/SKILL.md) §13**. With git: they land as unstaged modifications. After moving the plan out, the diff contains only the real code changes.)
...
After the PR is open, leave `@` as an empty child on top of the bookmark so the next edit starts clean (see **[jujutsu](../jujutsu/SKILL.md) §9b**):
...
Each subagent is workspace-isolated by prompt, so the "no two subagents edit the same file" rule from **[subagent-review-main-agent-address](../subagent-review-main-agent-address/SKILL.md) §8** is automatically satisfied — their workspaces don't overlap. With jj, different workspaces can even point at different bookmarks simultaneously because each workspace has an independent `@`. With git worktrees, each has its own HEAD, so the same property holds.

---

## 11. End-to-end checklist

1. Read the ticket, evaluate, transition to "In Development" (see **[jira-story-to-pr-workflow](../jira-story-to-pr-workflow/SKILL.md)** §1–§4).
...
7. Review the subagent's diff directly (`jj diff` or `git diff`); triage per **[subagent-review-main-agent-address](../subagent-review-main-agent-address/SKILL.md) §4**. Run project checks.
...
| Topic | Skill |
|-------|-------|
| jj change/bookmark/push/squash | **[jujutsu](../jujutsu/SKILL.md)** |
| Ticket read, evaluate, transition, naming | **[jira-story-to-pr-workflow](../jira-story-to-pr-workflow/SKILL.md)** |
| Conventional commits, pre-commit checks | **[commits-and-pre-commit-checks](../commits-and-pre-commit-checks/SKILL.md)** |
| Subagent review loop, parallel subagents, background-agent stall | **[subagent-review-main-agent-address](../subagent-review-main-agent-address/SKILL.md)** |
| PR title/description format | **[pr-title-and-description](../pr-title-and-description/SKILL.md)** |
